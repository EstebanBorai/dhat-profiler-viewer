...
function onLoad() {
  ...
  // Creates custom header to notice the fork of the original source to the user
  const customHeader = appendElement(document.body, "div", "section");
  const titleEl = appendElement(customHeader, "h1");
  const subtitleEl = appendElement(customHeader, "p");

  titleEl.innerText = "DHAT Viewer v3.17.0";
  subtitleEl.innerHTML = `This is a fork from <a href="https://www.valgrind.org/downloads/current.html#current" target="_blank">Valgrind</a>'s`;
